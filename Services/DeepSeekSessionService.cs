...
public sealed class DeepSeekSessionService : IDisposable
{
    ...
    public Task<(bool Ok, string Body, bool SessionInvalid)> FetchAsync(string relativePath)
        => _dispatcher.InvokeAsync(() => FetchAsyncCore(relativePath)).Task.Unwrap();
    ...
    private async Task<(bool Ok, string Body, bool SessionInvalid)> FetchAsyncCore(string relativePath, bool post = false)
    {
        var wv2 = GetWebView();
        ...
        if (wv2.Source is null || !wv2.Source.StartsWith(ConsoleUrl, StringComparison.OrdinalIgnoreCase))
        {
            ...
            try
            {
                ...
                var done = await Task.WhenAny(navTcs.Task, Task.Delay(20000));
        ...
        try
        {
            ...
            var completed = await Task.WhenAny(execTask, Task.Delay(20000));
        ...
        if (string.IsNullOrEmpty(text) || text == "null" || text == "\"null\"") return (false, "", false);
        ...
        {
            if (status == 401) return (false, "", true);
            if (status == 429) return (false, "", false);
            body = body[(sep + 2)..];
        }
    ...
    public Task<(bool Ok, string Body)> FetchUsageViaPageAsync(string navigateTo, string pathFilter)
        => _dispatcher.InvokeAsync(() => FetchUsageViaPageCoreAsync(navigateTo, pathFilter)).Task.Unwrap();

    /// <summary>
    /// 页面自身请求捕获：导航到目标页，拦截其发出的 API 请求响应（页面请求带完整风控上下文，
    /// 注入的 fetch/XHR 会被静默返回空对象）。返回匹配路径的响应体。
    /// </summary>
    private async Task<(bool Ok, string Body)> FetchUsageViaPageCoreAsync(string navigateTo, string pathFilter)
    {
        var wv2 = GetWebView();
        ...
        async Task ReadResponseAsync(CoreWebView2WebResourceResponseView resp)
        {
            ...
            try
            {
                ...
                if (stream is null) { Logger.Log("page capture: content null"); return; }
                using var reader = new System.IO.StreamReader(stream);
                var body = await reader.ReadToEndAsync();
                Logger.Log($"page capture body: {(body.Length > 400 ? body[..400] : body)}");
                // 非 { 开头视为异常碎片（双保险），丢弃后等下一个响应
                if (body.Length > 0 && body.TrimStart().StartsWith('{'))
                    tcs.TrySetResult(body);
                else
                    Logger.Log("page capture: 非 JSON 开头，疑似读取碎片，已忽略");
            }
            catch (Exception ex)
            {
        ...
        try
        {
            ...
            var body = await tcs.Task.WaitAsync(TimeSpan.FromSeconds(40));
    ...
    {
        var (ok, body, _) = await FetchAsync("/api/v0/users/get_user_summary");
        if (ok && body.Length > 0)
            Logger.Log($"deepseek summary: {(body.Length > 500 ? body[..500] : body)}");
        if (ok && body == "{}")
        {
            var pageUrl = await _dispatcher.InvokeAsync(() => GetWebView().Source);
            Logger.Log($"deepseek page url: {pageUrl}");
            var (okK, bodyK, _) = await FetchAsync("/api/v0/users/get_api_keys");
    ...
    private async Task<bool> RestoreCookiesCoreAsync()
    {
        try
        {
            ...
            var ok = await CheckSessionAsync();
