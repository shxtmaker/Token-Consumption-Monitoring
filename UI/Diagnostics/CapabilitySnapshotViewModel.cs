...
public sealed class CapabilitySnapshotViewModel : INotifyPropertyChanged
{
    ...
    public bool HasReportedUsage => !string.IsNullOrEmpty(ReportedUsageLabel) && TotalTokens > 0;
    ...
    public bool HasCapabilities => HasWindows || HasReportedUsage || HasBalance;
    ...
    public void Update(CapabilitySnapshot snapshot, bool showDailyUsage)
    {
        ...
        IsStale = snapshot.Status == SnapshotStatus.Stale;
        ...
        Notify(nameof(HasWindows), nameof(HasReportedUsage), nameof(HasBalance), nameof(HasProbeMessage),
            nameof(HasCapabilities), nameof(ReportedUsageLabel), nameof(BalanceLabel), nameof(StatusLabel),
            nameof(TotalCostLabel), nameof(StatusMessage), nameof(IsStale), nameof(IsAuthRequired), nameof(IsProbeOnly));
    }

    public void Reset()
    {
        Windows.Clear(); ModelRows.Clear(); DeepSeekRows.Clear();
        ReportedUsageLabel = ""; BalanceLabel = ""; StatusMessage = "";
        TotalTokens = 0; StatusLabel = "暂无数据";
        IsStale = IsAuthRequired = IsProbeOnly = false;
        Notify(nameof(HasWindows), nameof(HasReportedUsage), nameof(HasBalance), nameof(HasCapabilities),
            nameof(ReportedUsageLabel), nameof(BalanceLabel), nameof(StatusLabel), nameof(TotalCostLabel));
    }
